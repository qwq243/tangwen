...
(function () {
  ...
  /* 空闲补图的范围。这是**半径**，实际补的是 IDLE_SPAN*2+1 张，再被全库条数截断
     —— 所以桌面的 34 在 44 卷时就是「全补」，不用跟着卷数改。
     桌面上全补无所谓；手机上「全补」等于把几十张封面一起解进内存 —— 那是这个页面
     最大的一块占用，也是最容易被系统回收掉的一块。所以手机上只补前后各 8 卷：
     顺着滑、搜索跳卷都还在圈里，常驻内存少掉一大半。 */
  ...
  /* ---------------- 搜剧本 ----------------
     ...
     卷目一多，顺着滑太慢，顶上给一条搜索。索引四样：卷名、卷号（02 / 2 都认）、
  ...
  fetch("/api/puzzles")
    ...
    .then((data) => {
      ...
      // 引导卡末尾那句卷数也在这儿补。写死在 HTML 里迟早过期 —— 它曾经停在
      // 「三十四卷」，语料后来扩到 44 卷，一直没人发现（玩家可见的错数字）。
      $("tourVols").textContent = String(puzzles.length);
      paintStrip();
      setOffset(0, false);
